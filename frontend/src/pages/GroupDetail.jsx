...
export default function GroupDetail() {
  ...
  const handleAddNote = async (e) => {
    ...
    try {
      ...
      setMsg('Error: ' + (err.response?.data?.detail || 'Failed to share note.'));
  ...
  const handleRemoveNote = async (gnId) => {
    ...
    try {
      ...
      setMsg('Error: ' + (err.response?.data?.detail || 'You do not have permission to remove this note.'));
    }
  };
  ...
  const handleLeave = async () => {
    ...
    catch (err) { setMsg('Error: ' + (err.response?.data?.detail || 'Failed to leave.')); }
  };
  ...
  const handleDelete = async () => {
    ...
    catch (err) { setMsg('Error: ' + (err.response?.data?.detail || 'Failed to delete.')); }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-page)' }}>
      ...
      <main className="container page-enter" style={{ padding: '2rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            {group.my_role !== 'owner' && (
              ...
              onMouseOver={e => e.currentTarget.style.background = 'var(--bg-muted)'}
              onMouseOut={e => e.currentTarget.style.background = 'transparent'}
              onClick={handleLeave}>Leave Group</button>
            )}
            {group.my_role === 'owner' && (
              <button
              style={{
                padding: '0.4rem 1rem', fontSize: '0.8125rem', fontWeight: 600,
                border: '1.5px solid var(--error-border)', borderRadius: 'var(--radius-sm)',
                background: 'transparent', color: 'var(--error)',
                cursor: 'pointer', transition: 'all 0.15s',
              }}
              onMouseOver={e => e.currentTarget.style.background = 'var(--error-bg)'}
              onMouseOut={e => e.currentTarget.style.background = 'transparent'}
              onClick={handleDelete}>Delete Group</button>
            )}
          </div>
        </div>
        ...
        {/* Alert */}
        {msg && (
          <div className={`alert ${msg.startsWith('Note') ? 'alert-success' : 'alert-error'}`} style={{ marginBottom: '1.25rem' }}>
            {msg}
          </div>
        )}
                ...
                {group.notes.map(note => (
                  <div key={note.group_note_id} className="card" style={{ padding: '1rem 1.25rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        <Link to={`/notes/${note.audio_file_id}`} className="btn btn-success btn-sm">View Notes</Link>
                        {/* Only the person who added the note or the group owner may remove it */}
                        {(group.my_role === 'owner' || note.added_by_id === group.my_user_id) && (
                          <button
                            onClick={() => handleRemoveNote(note.group_note_id)}
                  ...
                  <div key={m.user_id} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '0.875rem', fontWeight: 600 }}>
                        {m.name} {m.role === 'owner' && <span style={{ fontSize: '0.7rem', color: 'var(--brand)', fontWeight: 600 }}>Owner</span>}
                      </div>
