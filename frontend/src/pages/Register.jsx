...
// Registration: name + email + password → OTP verification → auto-login
// Also: Continue with Google via GIS renderButton
...
export default function Register() {
  ...
  // ── Render Google button into ref div ─────────────────────────
  useEffect(() => {
    ...
    const init = () => {
      ...
      window.google.accounts.id.initialize({
        ...
        callback: async (response) => {
          if (!response?.credential) { setGoogleErr('No credential from Google.'); return }
          ...
          try {
            ...
            setGoogleErr(err.response?.data?.detail || 'Sign-up failed. Please try again.')
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setResendMsg('New code sent!'); setOtp(['','','','','','']); otpRefs.current[0]?.focus()
    } catch (err) { setStep2Err(err.response?.data?.detail || 'Could not resend. Try again.') }
  }
          ...
          <>
            ...
            <form onSubmit={handleRegister} noValidate>
              ...
              <div className="form-group">
                ...
                <input id="reg-name" type="text" className="form-input" placeholder="Udaya Kumar"
