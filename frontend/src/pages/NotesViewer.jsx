...
export default function NotesViewer() {
  ...
  const handleTranslate = async () => {
    ...
    } catch (e) { setTranslatedText('Translation failed: ' + e.message); }
        ...
        <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
          ...
          {activeView === 'notes' && (
            <div style={{ height: '100%', overflowY: 'auto', padding: '28px 32px', background: '#F4F5F7', animation: 'fadeIn 0.2s ease' }}>
              <div style={{ maxWidth: 740, margin: '0 auto', background: '#fff', borderRadius: 12, padding: '40px 48px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', border: '1px solid #E5E7EB' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <span style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.1em', color: '#2563EB', textTransform: 'uppercase' }}>Lecture Notes</span>
                  <span style={{ fontSize: '0.8rem', color: '#9CA3AF' }}>{today}</span>
                </div>
                <h1 style={{ fontSize: '2rem', fontWeight: 800, color: '#111827', lineHeight: 1.25, margin: '0 0 0.875rem' }}>{notes.title || 'Lecture Notes'}</h1>
                <div style={{ display: 'flex', gap: '1.25rem', fontSize: '0.8125rem', color: '#6B7280', paddingBottom: '1.25rem', borderBottom: '1px solid #E5E7EB', flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                {/* ── 1. OVERVIEW ─────────────────────────────────── */}
                {notes.summary && (
                  <div style={{ margin: '1.75rem 0 0', padding: '1.25rem 1.5rem', background: '#F0F7FF', borderRadius: 10, border: '1px solid #BFDBFE' }}>
                    <div style={{ fontSize: '0.68rem', fontWeight: 800, letterSpacing: '0.12em', color: '#2563EB', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Overview</div>
                    <p style={{ fontSize: '0.9625rem', lineHeight: 1.85, color: '#1E3A8A', margin: 0 }}>{notes.summary}</p>
                  </div>
                )}

                {/* ── 2. SECTIONS: Heading → Definition → Key Points ── */}
                {notes.sections?.map((sec, idx) => (
                  <div key={idx} style={{ borderTop: '1px solid #F3F4F6', paddingTop: '1.75rem', marginTop: '1.75rem' }}>
                    <h3 style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: '1.1rem', fontWeight: 800, color: '#111827', marginBottom: '0.875rem' }}>
                    ...
                    <p style={{ fontSize: '0.9375rem', lineHeight: 1.85, color: '#374151', margin: '0 0 1rem 0' }}>{sec.definition || sec.content}</p>
                    {sec.key_points?.length > 0 && (
                      <div style={{ background: '#FAFAFA', borderRadius: 8, border: '1px solid #F0F0F0', padding: '0.875rem 1.125rem' }}>
                        <div style={{ fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.1em', color: '#9CA3AF', textTransform: 'uppercase', marginBottom: '0.6rem' }}>Key Points</div>
                        <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                          {sec.key_points.map((pt, pi) => (
                            <li key={pi} style={{ display: 'flex', gap: 9, alignItems: 'flex-start' }}>
                              <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#2563EB', flexShrink: 0, marginTop: '0.5rem' }} />
                              <span style={{ fontSize: '0.875rem', lineHeight: 1.72, color: '#374151' }}>{pt}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                ...
                {/* ── 3. TOP-LEVEL KEY CONCEPTS ────────────────────── */}
                {notes.key_points?.length > 0 && (<div style={{ borderTop: '1px solid #F3F4F6', paddingTop: '1.75rem', marginTop: '1.75rem' }}><div style={{ fontSize: '0.68rem', fontWeight: 800, letterSpacing: '0.12em', color: '#2563EB', textTransform: 'uppercase', marginBottom: '0.875rem' }}>Key Concepts</div><ol style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.55rem' }}>{notes.key_points.map((pt, i) => <li key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}><span style={{ width: 22, height: 22, borderRadius: '50%', background: '#2563EB', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', fontWeight: 700, flexShrink: 0, marginTop: 2 }}>{i + 1}</span><span style={{ fontSize: '0.9rem', lineHeight: 1.72, color: '#374151' }}>{pt}</span></li>)}</ol></div>)}

                {/* ── FULL TRANSCRIPT (collapsible) ─────────────────── */}
                {fullTranscript && (<div style={{ borderTop: '1px solid #F3F4F6', paddingTop: '1.5rem', marginTop: '1.5rem' }}><details><summary style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.1em', color: '#9CA3AF', textTransform: 'uppercase', cursor: 'pointer', marginBottom: '0.75rem' }}>Full Transcript</summary><p style={{ fontSize: '0.875rem', lineHeight: 1.85, color: '#6B7280', whiteSpace: 'pre-wrap', margin: 0 }}>{fullTranscript}</p></details></div>)}
              </div>
            </div>
          )}
            ...
            <div style={{ height: '100%', overflowY: 'auto', padding: '24px 28px', background: '#1E293B', animation: 'fadeIn 0.2s ease' }}>
              <div style={{ maxWidth: 760, margin: '0 auto' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
                  <div><h2 style={{ color: '#F1F5F9', fontWeight: 800, fontSize: '1.25rem', margin: '0 0 4px' }}>Raw Transcripts</h2><p style={{ color: '#94A3B8', fontSize: '0.8rem', margin: 0 }}>Chunk-by-chunk transcription output</p></div>
          ...
          {activeView === 'translate' && (
            <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'linear-gradient(135deg,#EFF6FF 0%,#F0F9FF 100%)', animation: 'fadeIn 0.2s ease' }}>
              <div style={{ padding: '28px 36px', borderBottom: '1px solid #BFDBFE', flexShrink: 0 }}>
                <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#1E3A8A', margin: '0 0 6px' }}>Translate Notes</h2>
                <p style={{ color: '#3B82F6', fontSize: '0.875rem', margin: 0 }}>Powered by local AI (Ollama) — translates your full lecture notes</p>
              </div>
                ...
                <div style={{ maxWidth: 680, margin: '0 auto' }}>
                  <div style={{ background: '#fff', borderRadius: 14, padding: '28px', border: '1.5px solid #BFDBFE', boxShadow: '0 2px 12px rgba(37,99,235,0.08)', marginBottom: '1.5rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'center' }}>
                      <select value={targetLang} onChange={e => setTargetLang(e.target.value)} style={{ flex: 1, minWidth: 180, padding: '10px 14px', border: '1.5px solid #BFDBFE', borderRadius: 10, fontSize: '0.9rem', background: '#F8FAFF', outline: 'none', color: '#1E3A8A', fontWeight: 600 }}>
                        {LANGUAGES.map(l => <option key={l.code} value={l.code}>{l.name}</option>)}
                      </select>
                      <button onClick={handleTranslate} disabled={translating} style={{ padding: '10px 28px', background: '#2563EB', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: '0.9rem', cursor: translating ? 'wait' : 'pointer', boxShadow: '0 2px 8px rgba(37,99,235,0.3)' }}>
                        {translating ? 'Translating...' : 'Translate Now'}
                      </button>
                      {translatedText && <button onClick={() => setTranslatedText('')} style={{ padding: '10px 16px', background: 'transparent', border: '1.5px solid #BFDBFE', borderRadius: 10, cursor: 'pointer', color: '#6B7280', fontWeight: 600 }}>Clear</button>}
                    </div>
                  </div>
                  {translatedText && (
                  ...
                  {!translatedText && !translating && (
                    <div style={{ textAlign: 'center', padding: '3rem', color: '#93C5FD' }}>
                      ...
                      <p style={{ fontSize: '0.9rem' }}>Select a language and click Translate Now to convert your notes</p>
                    </div>
                  )}
                  {translating && (
                    <div style={{ textAlign: 'center', padding: '3rem', color: '#3B82F6' }}>
                      <div className="spinner" style={{ margin: '0 auto 1rem' }} />
                      <p>Translating your notes...</p>
                    </div>
                  )}
                </div>
              </div>
