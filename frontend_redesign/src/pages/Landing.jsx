    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 20, padding: '2.5rem 2rem', width: '100%', maxWidth: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.25)', position: 'relative' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: 'linear-gradient(135deg,#D97706,#F59E0B)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', margin: '0 auto 0.75rem' }}>🎙️</div>
        ...
        {err && <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', color: '#DC2626', borderRadius: 8, padding: '0.625rem 0.875rem', fontSize: '0.875rem', marginBottom: '1rem' }}>⚠️ {err}</div>}
        {step === 1 ? (
      ...
      <section className="lv2-hero">
        <div className="lv2-hero-left">
          ...
          <div className="lv2-cta-row">
            {isLoggedIn ? (
              <Link to="/upload" className="lv2-btn-primary">⬆ Drop Lecture Audio Now</Link>
            ) : (
              <button onClick={() => setModal('register')} className="lv2-btn-primary" style={{ cursor: 'pointer' }}>⬆ Drop Lecture Audio Now</button>
            )}
            {isLoggedIn ? (
